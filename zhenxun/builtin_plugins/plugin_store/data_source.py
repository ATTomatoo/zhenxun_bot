...
from zhenxun.utils.utils import is_binary_file, is_number, win_on_rm_error
...
class StoreManager:
    ...
    def _get_source_order(source: str | None) -> tuple[RepoType, ...]:
        ...
        默认统一使用阿里云，失败后回退 GitHub。显式指定 -s 时只使用对应源。
        """
        if source is None:
    ...
    @staticmethod
    def _get_repository_branch(repo_url: str, default_branch: str) -> str:
    ...
    async def _download_plugin_to_staging(
        ...
        branch_source_url = plugin_info.github_url or repo_url
        branch = cls._get_repository_branch(branch_source_url, default_branch)
        ...
        sparse_path = repository_plugin_path or "."
        result = await RepoFileManager.download_files(
            repo_url,
            download_files,
            branch,
            repo_type=repo_type,
            sparse_path=sparse_path if repo_type == RepoType.ALIYUN else None,
            target_dir=staging_root if repo_type == RepoType.ALIYUN else None,
        )
        if not result.success:
            raise PluginStoreException(result.error_message or "未知下载错误")

        for source_path, staged_path in download_files:
            if not staged_path.is_file():
                raise PluginStoreException(f"插件文件下载不完整: {source_path}")
            if is_binary_file(source_path) and staged_path.stat().st_size == 0:
